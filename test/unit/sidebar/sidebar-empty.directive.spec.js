...
describe('Directive: sidebarEmpty', function() {
  ...
  describe('suggested subreddits', function() {
    it('should contain at least 4 items of certain structure', function() {
      compile(function(scope) {
        ...
        expect(scope.$ctrl.suggestedSubreddits[0]).toEqual(
  ...
  describe('when clicked', () => {
    it('should call onSubredditClick with correct object', () => {
      ...
      compile(
        parentScope,
        {'on-subreddit-click': 'onSubredditClick(subreddit)'},
        function(scope, element) {
          element
            .find('.sidebar-suggestions  button')
            .get(0)
            .click();
          expect(parentScope.onSubredditClick).toHaveBeenCalledWith(
            scope.$ctrl.suggestedSubreddits[0].name
          );
        }
      );
    });
